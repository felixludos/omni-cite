...
class OneDriveProcess(fig.Configurable):
	...
	def get_meta(self, paths):
		...
		if isinstance(out, list):
			return [r['body']['link']['webUrl'] for r in out]
		...
		resp = requests.post('https://graph.microsoft.com/v1.0/$batch', json=body,
		                     headers={'content-type': 'application/json', **self._onedrive_header})
	...
	def share_files(self, paths, mode='view'):
		...
		if isinstance(out, list):
			try:
				...
				links = [r['body']['link']['webUrl'] for r in out]
				if mode == 'download':
					return [link.replace('embed', 'download') for link in links]
				return links
			except KeyError:
				return out
		return out
